...
namespace Bootstrapper
{
    ...
    public:
        ...
        void ArpEntryExeInstallTest()
        {
            ...
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"TestExe", TRUE, BURN_CACHE_PACKAGE_TYPE_REQUIRED, BURN_CACHE_PACKAGE_TYPE_NONE);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"TestExe");
        ...
        void ArpEntryExeUninstallTest()
        {
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 1);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"TestExe", FALSE, BURN_CACHE_PACKAGE_TYPE_NONE, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"TestExe");
            Assert::Equal(dwIndex, pPlan->cCacheActions);
            ...
            Assert::Equal(143724ull, pPlan->qwCacheSizeTotal);
        ...
        void MsiTransactionUninstallTest()
        {
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 9);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PackageA", FALSE, BURN_CACHE_PACKAGE_TYPE_NONE, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PackageA");
            Assert::Equal(dwIndex, pPlan->cCacheActions);
            ...
            Assert::Equal(185118ull, pPlan->qwCacheSizeTotal);
        ...
        void MultipleBundlePackageInstallTest()
        {
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 1);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PackageA", TRUE, BURN_CACHE_PACKAGE_TYPE_REQUIRED, BURN_CACHE_PACKAGE_TYPE_OPTIONAL);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PackageA");
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 6);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PackageB", TRUE, BURN_CACHE_PACKAGE_TYPE_REQUIRED, BURN_CACHE_PACKAGE_TYPE_OPTIONAL);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PackageB");
            Assert::Equal(dwIndex, pPlan->cCacheActions);
        ...
        [Fact]
        void MultipleBundlePackageRepairTest()
        {
            HRESULT hr = S_OK;
            BURN_ENGINE_STATE engineState = { };
            BURN_ENGINE_STATE* pEngineState = &engineState;
            ...
            ValidateExecuteCheckpoint(pPlan, fRollback, dwIndex++, dwExecuteCheckpointId++);
            ValidateExecuteWaitCachePackage(pPlan, fRollback, dwIndex++, L"PackageB");
            ValidateExecuteBundlePackage(pPlan, fRollback, dwIndex++, L"PackageB", BOOTSTRAPPER_ACTION_STATE_REPAIR, L"{35192ED0-C70A-49B2-9D12-3B1FA39B5E6F}");
            ValidateExecutePackageDependency(pPlan, fRollback, dwIndex++, L"PackageB", L"{35192ED0-C70A-49B2-9D12-3B1FA39B5E6F}", registerActions1, 1);
            ValidateExecuteCheckpoint(pPlan, fRollback, dwIndex++, dwExecuteCheckpointId++);
            ValidateExecuteCheckpoint(pPlan, fRollback, dwIndex++, dwExecuteCheckpointId++);
            ValidateExecuteRollbackBoundaryEnd(pPlan, fRollback, dwIndex++);
        ...
        [Fact]
        void MultipleBundlePackageUninstallTest()
        {
            HRESULT hr = S_OK;
            BURN_ENGINE_STATE engineState = { };
            BURN_ENGINE_STATE* pEngineState = &engineState;
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 1);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PackageB", FALSE, BURN_CACHE_PACKAGE_TYPE_OPTIONAL, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PackageB");
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 6);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PackageA", FALSE, BURN_CACHE_PACKAGE_TYPE_OPTIONAL, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PackageA");
            Assert::Equal(dwIndex, pPlan->cCacheActions);
            ...
            UINT uIndex = 0;
            ValidatePlannedProvider(pPlan, uIndex++, L"{35192ED0-C70A-49B2-9D12-3B1FA39B5E6F}", NULL);
            ValidatePlannedProvider(pPlan, uIndex++, L"{7506235A-7C59-4750-82C7-EB460A87ED3A}", NULL);
            ValidatePlannedProvider(pPlan, uIndex++, L"{B39CEE4D-CCD7-4797-BE3A-6613BD1DC4BE}", NULL);
            Assert::Equal(uIndex, pPlan->cPlannedProviders);
            ...
            ValidateNonPermanentPackageExpectedStates(&pEngineState->packages.rgPackages[2], L"PackageB", BURN_PACKAGE_REGISTRATION_STATE_ABSENT, BURN_PACKAGE_REGISTRATION_STATE_ABSENT);
        }
        ...
        void SingleExeInstallTest()
        {
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 1);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"ExeA", TRUE, BURN_CACHE_PACKAGE_TYPE_REQUIRED, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"ExeA");
            Assert::Equal(dwIndex, pPlan->cCacheActions);
        ...
        void SingleExeLayoutTest()
        {
            ...
            Assert::Equal<BOOL>(FALSE, pPlan->fDisallowRemoval);
            Assert::Equal<BOOL>(FALSE, pPlan->fDowngrade);
            Assert::Equal<DWORD>(0, pPlan->dwRegistrationOperations);
            Assert::True(0 != pPlan->sczLayoutDirectory);
            Assert::NotEqual<WCHAR>(0, *pPlan->sczLayoutDirectory);
            ...
            fRollback = FALSE;
            dwIndex = 0;
            ValidateCacheLayoutBundle(pPlan, fRollback, dwIndex++);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"NetFx48Web", TRUE, BURN_CACHE_PACKAGE_TYPE_NONE, BURN_CACHE_PACKAGE_TYPE_NONE);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"ExeA", TRUE, BURN_CACHE_PACKAGE_TYPE_NONE, BURN_CACHE_PACKAGE_TYPE_NONE);
            Assert::Equal(dwIndex, pPlan->cCacheActions);
            ...
            Assert::Equal(4321686ull, pPlan->qwCacheSizeTotal);
            ...
            Assert::Equal(2ul, pEngineState->packages.cPackages);
            ValidateNonPermanentPackageExpectedStates(&pEngineState->packages.rgPackages[1], L"ExeA", BURN_PACKAGE_REGISTRATION_STATE_ABSENT, BURN_PACKAGE_REGISTRATION_STATE_ABSENT);
        }
        ...
        void SingleMsuInstallTest()
        {
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 1);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"test.msu", TRUE, BURN_CACHE_PACKAGE_TYPE_REQUIRED, BURN_CACHE_PACKAGE_TYPE_NONE);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"test.msu");
            Assert::Equal(dwIndex, pPlan->cCacheActions);
        ...
        void SlipstreamUninstallTest()
        {
            ...
            dwIndex = 0;
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 1);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PatchA", FALSE, BURN_CACHE_PACKAGE_TYPE_NONE, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PatchA");
            ValidateCacheCheckpoint(pPlan, fRollback, dwIndex++, 6);
            ValidateCachePackage(pPlan, fRollback, dwIndex++, L"PackageA", FALSE, BURN_CACHE_PACKAGE_TYPE_NONE, BURN_CACHE_PACKAGE_TYPE_REQUIRED);
            ValidateCacheSignalSyncpoint(pPlan, fRollback, dwIndex++, L"PackageA");
        ...
        void UnuninstallableExePackageForceAbsentTest()
        {
            ...
            Assert::Equal(131072ull, pPlan->qwCacheSizeTotal);
    ...
    private:
        ...
        void ValidateCacheLayoutBundle(
            __in BURN_PLAN* pPlan,
            __in BOOL fRollback,
            __in DWORD dwIndex
            )
        {
            BURN_CACHE_ACTION* pAction = ValidateCacheActionExists(pPlan, fRollback, dwIndex);
            Assert::Equal<DWORD>(BURN_CACHE_ACTION_TYPE_LAYOUT_BUNDLE, pAction->type);
        }
        ...
        DWORD ValidateCachePackage(
            ...
            __in BOOL fVital,
            __in BURN_CACHE_PACKAGE_TYPE executeCacheType,
            __in BURN_CACHE_PACKAGE_TYPE rollbackCacheType
            )
        {
            BURN_CACHE_ACTION* pAction = ValidateCacheActionExists(pPlan, fRollback, dwIndex);
            ...
            Assert::Equal<BOOL>(fVital, pAction->package.pPackage->fCacheVital);
            Assert::Equal<DWORD>(executeCacheType, pAction->package.pPackage->executeCacheType);
            Assert::Equal<DWORD>(rollbackCacheType, pAction->package.pPackage->rollbackCacheType);
            return dwIndex + 1;
        }
            ...
            )
        {
            ...
            NativeAssert::StringEqual(wzPackageId, pAction->syncpoint.pPackage->sczId);
        }
